...
# ---------------------------------------------------------------------------
# S8.2 - LAMINA D'AGUA INCLINADA (AP1.2, Modulo 12)
# ---------------------------------------------------------------------------

def perfil_do_casco(tab: Tabela):
    """
    Contorno longitudinal do casco a partir da tabela de cotas.

    Para cada baliza, devolve a menor e a maior altura em que existe meia-boca:
    a linha inferior e o fundo, a superior e a borda. Nao e o perfil real da roda
    de proa nem do cadaste, e sim o envelope do que a tabela descreve.
    """
    fundo, borda, xs = [], [], []
    Y = np.nan_to_num(tab.Y, nan=0.0)
    for i in range(tab.n_est):
        com = np.where(Y[i] > 1e-9)[0]
        if not len(com):
            continue
        xs.append(float(tab.x[i]))
        fundo.append(float(tab.z[com[0]]))
        borda.append(float(tab.z[com[-1]]))
    return np.array(xs), np.array(fundo), np.array(borda)


def plot_lamina_dagua(tab: Tabela, eq: dict, origem="tabela", mostrar_centros=True):
    """
    Vista de perfil com a lamina d'agua inclinada resultante do calculo.

    A linha e tracada entre os pontos (AP, Ta) e (FP, Tf) calculados: nao ha nada
    de ilustrativo nela. Marca tambem a linha de base, as perpendiculares, o LCF,
    o LCB e o LCG, e anota o compasso e o sentido do trim.
    """
    fig, ax = plt.subplots(figsize=(11.5, 4.6))
    zb = z_base(tab)
    xs, fundo, borda = perfil_do_casco(tab)
    xa, xf = eq["xa"], eq["xf"]

    if len(xs) >= 2:
        ax.fill_between(xs, fundo, borda, color="#c9d9e4", alpha=0.55, zorder=1,
                        label="envelope do casco")
        ax.plot(xs, fundo, color="#20465e", lw=1.6, zorder=3)
        ax.plot(xs, borda, color="#20465e", lw=1.6, zorder=3)

    ax.axhline(zb, color="k", lw=1.2, zorder=2)
    ax.text(xa, zb, " Baseline", fontsize=7, va="bottom", ha="left", color="k")
    for x_, nome in ((xa, "AP"), (xf, "FP")):
        ax.axvline(x_, color="0.45", lw=1.0, ls="-.", zorder=2)
        ax.text(x_, zb, f" {nome}", fontsize=8, va="top", ha="center", color="0.3")

    Ta, Tf = eq["Ta"], eq["Tf"]
    if np.isfinite(Ta) and np.isfinite(Tf):
        za, zf = zb + Ta, zb + Tf
        ax.plot([xa, xf], [za, zf], color="#e8a33d", lw=2.6, zorder=6,
                label="lamina d'agua calculada")
        ax.fill_between([xa, xf], [zb, zb], [za, zf], color="#2a9df4",
                        alpha=0.16, zorder=1)
        ax.annotate(f"Ta = {fmt(Ta)} m", (xa, za), xytext=(6, 8),
                    textcoords="offset points", fontsize=9, color="#b5791f",
                    fontweight="bold")
        ax.annotate(f"Tf = {fmt(Tf)} m", (xf, zf), xytext=(-6, 8),
                    textcoords="offset points", fontsize=9, color="#b5791f",
                    fontweight="bold", ha="right")
        for x_, z_ in ((xa, za), (xf, zf)):
            ax.plot([x_, x_], [zb, z_], color="#b5791f", lw=1.0, ls=":", zorder=5)

    if mostrar_centros and np.isfinite(eq.get("T0", np.nan)):
        marcas = [("LCF", eq["LCF"], "#2a9df4", "o"),
                  ("LCB", eq["LCB"], "#0b6e4f", "s"),
                  ("LCG", eq["LCG"], "#d62728", "^")]
        alturas = np.linspace(0.18, 0.42, len(marcas))
        span = (borda.max() - zb) if len(borda) else 1.0
        for (nome, xv, cor, marca), frac in zip(marcas, alturas):
            if not np.isfinite(xv):
                continue
            zv = zb + frac * span
            ax.plot([xv], [zv], marker=marca, color=cor, ms=9, zorder=7,
                    markeredgecolor="white", markeredgewidth=1.0)
            ax.axvline(xv, color=cor, lw=0.9, ls="--", alpha=0.55, zorder=2)
            ax.annotate(f"{nome} = {fmt(converter_origem(xv, tab, origem))} m",
                        (xv, zv), xytext=(8, 0), textcoords="offset points",
                        fontsize=8, color=cor, va="center", fontweight="bold")

    sentido = eq.get("sentido", "")
    ax.set_title(f"Lamina d'agua  |  compasso t = {fmt(eq.get('t_cm'), 2)} cm "
                 f"({fmt(eq.get('t_m'), 4)} m)  |  {sentido}  |  "
                 f"angulo = {fmt(eq.get('theta'), 3)} graus",
                 fontsize=11, fontweight="bold")
    ax.set_xlabel("x (m) - cresce de re para vante", fontsize=9)
