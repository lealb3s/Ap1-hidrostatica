...
def gerar_relatorio_pdf(ctx: dict) -> bytes:
    ...
    # ---- 10-A. condicao de carga e equilibrio (AP1.2) ----------------------
    eq = ctx.get("eq")
    if eq:
        soma = ctx.get("soma_carga") or {}
        A(PageBreak())
        titulo("10-A. Condicao de carga e equilibrio longitudinal")
        texto(f"<b>Condicao:</b> {ctx.get('nome_condicao') or '(sem nome)'}. "
              "LCB, LCF, BM_l, KM e o proprio calado foram interpolados na Hydrostatic "
              "Table calculada por este aplicativo (secao 8); nenhum valor foi digitado "
              "nem fixado no codigo.")

        sub("10-A.1 Itens da condicao")
        if ctx.get("df_carga") is not None and len(ctx["df_carga"]):
            tabela(ctx["df_carga"], 4, 30)
        for o in (ctx.get("achados_carga") or []):
            texto(f"- {o}", "Nota")

        sub("10-A.2 Somatorio")
        texto("Delta = soma(w_i) ; LCG = soma(w_i.x_i)/Delta ; KG = soma(w_i.z_i)/Delta ; "
              "GG_0 = soma(rho_i.i_i)/Delta ; KG_corrigido = KG + GG_0")
        tabela(pd.DataFrame({
            "Grandeza": ["Deslocamento Delta (t)", "Soma dos momentos long. (t.m)",
                         "LCG (m)", "Soma dos momentos vert. (t.m)", "KG sem correcao (m)",
                         "Correcao de superficie livre GG_0 (m)", "KG corrigido (m)"],
            "Valor": [soma.get("delta"), soma.get("soma_mom_l"), soma.get("LCG"),
                      soma.get("soma_mom_v"), soma.get("KG"), soma.get("GG0"),
                      soma.get("KG_corr")]}), 4, 12)

        sub("10-A.3 Hidrostatica no calado de equilibrio")
        bsc = eq.get("busca") or {}
        texto(f"T_0 resolve rho.Vol(T) = Delta. Interpolando a Hydrostatic Table entre "
              f"T = {fmt(bsc.get('T_inf'))} m (Delta = {fmt(bsc.get('D_inf'))} t) e "
              f"T = {fmt(bsc.get('T_sup'))} m (Delta = {fmt(bsc.get('D_sup'))} t), "
              f"resulta T_0 = {fmt(eq.get('T0'))} m.")
        if bsc.get("extrapolou"):
            texto("ATENCAO: o deslocamento esta fora da faixa da Hydrostatic Table; "
                  "o valor foi extrapolado.", "Nota")
        tabela(pd.DataFrame({
            "Grandeza": ["T_0 (m)", "LCB (m)", "LCF (m)", "BM_l (m)", "KM_l (m)",
                         "GM_l (m)", "KM_t (m)", "GM_t = KM_t - KG corrigido (m)"],
            "Valor": [eq.get("T0"), eq.get("LCB"), eq.get("LCF"), eq.get("BML"),
                      eq.get("KML"), eq.get("GML"), eq.get("KMT"), eq.get("GMT")]}), 4, 12)

        sub("10-A.4 Momento de trim e compasso")
        modo = eq.get("modo_mtc", "aproximado")
        form_mtc = ("Delta.BM_l/(100L)" if modo == "aproximado" else "Delta.GM_l/(100L)")
        texto("<b>Convencao de sinais adotada:</b> x cresce da popa (AP) para a proa (FP), "
              "a mesma referencia da tabela de cotas. M_trim = Delta.(LCG - LCB); positivo "
              "significa LCG a vante do LCB, ou seja trim pela proa, com T_f &gt; T_a.")
        tabela(pd.DataFrame({
            "Grandeza": [f"MTC = {form_mtc} - modo {modo} (t.m/cm)", "LCG - LCB (m)",
                         "M_trim = Delta.(LCG - LCB) (t.m)",
                         "Compasso t = M_trim/MTC (cm)", "Angulo de trim (graus)"],
            "Valor": [eq.get("MTC"),
                      (eq.get("LCG") or 0.0) - (eq.get("LCB") or 0.0),
                      eq.get("M_trim"), eq.get("t_cm"), eq.get("theta")]}), 4, 12)
        texto(f"<b>Sentido:</b> {eq.get('sentido', '')}")

        sub("10-A.5 Calados nas perpendiculares")
        texto("Rotacao em torno do LCF, e nao da meia-nau, porque o LCF e o centro de "
              "rotacao do plano de flutuacao: "
              "T_a = T_0 - (LCF - x_AP)/L . t &nbsp;&nbsp; "
              "T_f = T_0 + (x_FP - LCF)/L . t")
        tabela(pd.DataFrame({
            "Grandeza": ["Braco a re: LCF - x_AP (m)", "Braco a vante: x_FP - LCF (m)",
                         "Calado a re T_a (m)", "Calado a vante T_f (m)",
                         "Conferencia T_f - T_a (cm) - deve igualar o compasso"],
            "Valor": [eq.get("braco_re"), eq.get("braco_vante"), eq.get("Ta"),
                      eq.get("Tf"),
                      ((eq.get("Tf") or 0.0) - (eq.get("Ta") or 0.0)) * 100.0]}), 4, 12)

        if ctx.get("df_resultado_carga") is not None:
            sub("10-A.6 Quadro-resumo")
            tabela(ctx["df_resultado_carga"], 4, 40)
        if ctx.get("img_lamina"):
            A(PageBreak())
            sub("10-A.7 Lamina d'agua resultante")
            figura("img_lamina", 0.99)
