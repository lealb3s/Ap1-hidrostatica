...
PADRAO = {
    ...
    "opt": {"rho": 1.025, "metodo_x": "auto", "metodo_z": "auto",
            ...
            "limite_calado": "util", "T_limite_manual": 0.0},
    "T_sel": 1.0,
    "carga": None, "nome_condicao": "", "soma": None, "eq": None,
    "achados_carga": None, "df_resultado_carga": None,
}
...
MODOS_LIMITE = {
    "util": "ate onde a tabela descreve o casco",
    "projeto": "ate o calado de projeto Td",
    "tabela": "ate o topo da tabela de cotas",
    "manual": "ate um valor que eu definir",
}


def limite_calado() -> float:
    """
    Maior calado que o aplicativo vai considerar, conforme a escolha da etapa 1.

    Sem esse limite o programa calcula ate o topo da tabela de cotas, que muitas
    vezes esta bem acima da condicao real de operacao: a tabela pode trazer linhas
    d'agua do convés ou de borda falsa, e o calado de projeto costuma ser bem menor.
    O resultado sao calados que a embarcacao nunca alcanca ocupando a tabela e as
    curvas.
    """
    t = st.session_state.get("tab")
    if t is None:
        return 1.0
    teto = float(H.calado_max(t))
    if teto <= 0:
        return 1.0
    o = opcoes()
    modo = o.get("limite_calado", "util")
    if modo == "tabela":
        return teto
    if modo == "projeto":
        Td = principais().get("Td") or 0.0
        return float(min(Td, teto)) if Td > 0 else float(H.calado_util(t))
    if modo == "manual":
        v = float(o.get("T_limite_manual") or 0.0)
        return float(min(v, teto)) if v > 0 else teto
    return float(min(H.calado_util(t), teto))
    ...
    with st.sidebar:
        ...
        if t is None:
            ...
            if Tmax > 1e-9:
                ...
                teto = calado_maximo()
                if Tmax < teto - 1e-9:
                    st.caption(f"limitado a {H.fmt(Tmax)} m de {H.fmt(teto)} m "
                               "possiveis (etapa 1)")
