...
from .comum import (W, exige_completa, botao_proximo, calado_maximo,
                    ...
                    assinatura_calculo, CHAVES_RESULTADO, limite_calado, MODOS_LIMITE)
...
def _calcular():
    ...
    st.caption(f"O calculo e repetido para uma sequencia de calados. A tabela de cotas "
               f"cobre ate {H.fmt(Tmax_d)} m; o limite escolhido na etapa 1 "
               f"({MODOS_LIMITE[opt.get('limite_calado', 'util')]}) e "
               f"{H.fmt(Tlim)} m.")
    if Tmax_d < 1e-6:
    ...
    with c1:
        Tmin = numero_seguro("Calado inicial (m)", 0.001, Tlim,
                             max(Tlim / 10, 0.001), passo=0.05, key="ht_tmin")
    with c2:
        Tmax = numero_seguro("Calado final (m)", 0.002, Tlim, Tlim,
                             passo=0.05, key="ht_tmax")
    with c3:
        dT = numero_seguro("Passo entre calados (m)", 0.001, max(Tlim, 0.002),
                           max(Tlim / 20, 0.01), passo=0.05, key="ht_dt")

    if Tmax <= Tmin:
        st.error("O calado final precisa ser maior que o inicial.")
        return
    if Tmin <= 1e-9:
        st.caption("A primeira linha da tabela sera o calado zero: volume e "
                   "deslocamento nulos, com BM, KM e coeficientes vazios.")
...
def _verificar(df_ht):
    ...
    for chave, esperado in [("VOL", "crescente"), ("DESL", "crescente"),
        ...
        v = v[np.isfinite(v)]          # o calado zero deixa celulas vazias de proposito
        if len(v) < 2:
            continue
        d = np.diff(v)
