...
if os.path.isdir(pasta):
    ref = None
    # compara so os arquivos que descrevem O MESMO casco em formatos diferentes;
    # a barcaca e o VLCC sao outros cascos e entram nos seus proprios testes
    for arq in sorted(os.listdir(pasta)):
        if not arq.startswith("exemplo_formato"):
            continue
...
# ============================================================================
print("\n[15] AP1.2 - condicao de carga, equilibrio e trim")

# --- somatorio de pesos: caso com solucao a mao ---------------------------
carga = pd.DataFrame({
    "Item": ["A", "B"],
    "Peso (t)": [10.0, 30.0],
    "LCG (m)": [5.0, 15.0],
    "VCG (m)": [2.0, 4.0],
    "i_sup_livre (m4)": [0.0, 0.0],
    "rho_fluido (t/m3)": [0.0, 0.0],
    "Observacao": ["", ""],
})
sm = g["somatorio"](carga)
checa("Delta = soma dos pesos", perto(sm["delta"], 40.0, 1e-12))
checa("LCG = (10*5 + 30*15)/40 = 12,5", perto(sm["LCG"], 12.5, 1e-12), f"{sm['LCG']}")
checa("KG = (10*2 + 30*4)/40 = 3,5", perto(sm["KG"], 3.5, 1e-12), f"{sm['KG']}")
checa("Sem tanques, GG0 = 0", perto(sm["GG0"], 0.0, 1e-12))

carga_sl = carga.copy()
carga_sl["i_sup_livre (m4)"] = [0.0, 20.0]
carga_sl["rho_fluido (t/m3)"] = [0.0, 1.0]
sm2 = g["somatorio"](carga_sl)
checa("GG0 = rho*i/Delta = 1*20/40 = 0,5", perto(sm2["GG0"], 0.5, 1e-12), f"{sm2['GG0']}")
checa("KG corrigido = KG + GG0", perto(sm2["KG_corr"], 4.0, 1e-12))
checa("Superficie livre nao altera o deslocamento", perto(sm2["delta"], 40.0, 1e-12))

# --- barcaca: solucao analitica do equilibrio -----------------------------
Lb, Bb, Db = 40.0, 10.0, 5.0
tb = g["barcaca_teste"](Lb, Bb, Db, 21, 21)
ob = dict(opt)
ob.update({"LPP": Lb, "B": Bb, "sub_vertical": 4, "rho": 1.0})
dfb, _ = g["tabela_hidrostatica"](tb, 0.0, 4.5, 0.25, ob)

# condicao centrada na meia-nau: nao pode haver trim
centrada = pd.DataFrame({
    "Item": ["carga"], "Peso (t)": [800.0], "LCG (m)": [20.0], "VCG (m)": [2.0],
    "i_sup_livre (m4)": [0.0], "rho_fluido (t/m3)": [0.0], "Observacao": [""]})
sc = g["somatorio"](centrada)
eqc = g["equilibrio"](tb, ob, sc, dfb)
checa("Barcaca: Delta = 800 t da calado T0 = 2 m (Vol = LBT)",
      perto(eqc["T0"], 2.0, 1e-4), f"T0={eqc['T0']}")
checa("Barcaca: LCG na meia-nau nao produz trim",
      abs(eqc["t_cm"]) < 1e-6, f"t={eqc['t_cm']} cm")
checa("Barcaca sem trim: Ta = Tf = T0",
      perto(eqc["Ta"], 2.0, 1e-4) and perto(eqc["Tf"], 2.0, 1e-4),
      f"Ta={eqc['Ta']} Tf={eqc['Tf']}")

# deslocando o LCG para vante, o navio precisa aproar
avante = centrada.copy()
avante["LCG (m)"] = [22.0]
sa = g["somatorio"](avante)
eqa = g["equilibrio"](tb, ob, sa, dfb)
checa("LCG a vante do LCB produz aproamento (Tf > Ta)",
      eqa["Tf"] > eqa["Ta"] and eqa["M_trim"] > 0,
      f"Ta={eqa['Ta']:.4f} Tf={eqa['Tf']:.4f}")
checa("Tf - Ta e exatamente o compasso",
      perto(eqa["Tf"] - eqa["Ta"], eqa["t_m"], 1e-9))
# barcaca: LCF na meia-nau, entao o calado medio deve continuar T0
checa("Barcaca: LCF na meia-nau mantem o calado medio igual a T0",
      perto(0.5 * (eqa["Ta"] + eqa["Tf"]), eqa["T0"], 1e-6),
      f"medio={0.5*(eqa['Ta']+eqa['Tf']):.6f} T0={eqa['T0']:.6f}")
# MTC analitico da barcaca: BM_l = L^2/(12T), MTC = Delta*BM_l/(100L) = rho*B*L^2/1200
mtc_ex = 1.0 * Bb * Lb ** 2 / 1200.0
checa("Barcaca: MTC confere com a solucao analitica",
      perto(eqa["MTC"], mtc_ex, 1e-3), f"{eqa['MTC']:.6f} vs {mtc_ex:.6f}")

# re: deve apopar, com sinal oposto e mesmo modulo
are = centrada.copy()
are["LCG (m)"] = [18.0]
eqr = g["equilibrio"](tb, ob, g["somatorio"](are), dfb)
checa("LCG a re do LCB produz apopamento (Ta > Tf)",
      eqr["Ta"] > eqr["Tf"] and eqr["M_trim"] < 0)
checa("Deslocar o LCG para os dois lados da o mesmo compasso em modulo",
      perto(abs(eqr["t_cm"]), abs(eqa["t_cm"]), 1e-6))

# --- validacao da condicao de carga ---------------------------------------
ruim = carga.copy()
ruim.loc[0, "Peso (t)"] = np.nan
checa("Validacao acusa celula vazia no peso",
      "CG-VAZIO" in [a.codigo for a in g["validar_condicao"](ruim)])
fora = carga.copy()
fora["LCG (m)"] = [500.0, 15.0]
checa("Validacao acusa LCG fora do casco",
      "CG-LCG" in [a.codigo for a in g["validar_condicao"](fora, tb)])
checa("Condicao valida nao gera erro",
      not any(a.nivel == "ERRO" for a in g["validar_condicao"](carga, tb)))

# --- busca do calado por deslocamento --------------------------------------
busca = g["calado_por_deslocamento"](dfb, 800.0)
checa("Calado interpolado na Hydrostatic Table", perto(busca["T0"], 2.0, 1e-3),
      f"{busca['T0']}")
checa("Deslocamento fora da faixa e sinalizado",
      g["calado_por_deslocamento"](dfb, 99999.0)["extrapolou"])
ref = g["calado_por_busca"](tb, ob, 800.0)
checa("Busca direta converge para o mesmo calado",
      ref["convergiu"] and perto(ref["T"], 2.0, 1e-5), f"{ref['T']}")




# ============================================================================
print("\n[16] Leitura de condicao de carga vinda de planilha")

import io as _io16

# planilha no formato tipico: colunas de momento ja calculadas, linha de TOTAL
# no fim e uma nota de rodape logo abaixo
_linhas16 = [
    ["Item / Denominacao", "Peso (t)", "LCG (m)", "VCG (m)", "Observacao",
     "Momento longitudinal (t.m)", "Momento vertical (t.m)"],
    ["Embarcacao leve", 48.0, 8.0, 1.25, "Peso leve", 384.0, 60.0],
    ["Combustivel", 7.0, 6.0, 0.75, "Tanque 100%", 42.0, 5.25],
    ["Carga de conves", 5.0, 13.5, 2.0, "Carga", 67.5, 10.0],
    ["TOTAL / RESULTADO", 60.0, None, None, None, 493.5, 75.25],
    ["LCG calculado (m)", 8.225, None, None, None, None, None],
    ["Referencia: AP = 0,000 m; FP = 19,190 m", None, None, None, None, None, None],
]
_buf16 = _io16.BytesIO()
pd.DataFrame(_linhas16).to_excel(_buf16, index=False, header=False)
...
class _Falso16:
    ...
    def getvalue(self):
        return _buf16.getvalue()


df16, notas16 = g["ler_condicao"](_Falso16())
checa("Le apenas os itens, sem a linha de total nem a nota de rodape",
      len(df16) == 3, f"{len(df16)} linhas: {list(df16['Item'])}")
sm16 = g["somatorio"](df16)
checa("Delta nao sai dobrado pela linha de TOTAL",
      perto(sm16["delta"], 60.0, 1e-9), f"{sm16['delta']}")
checa("LCG confere com o total declarado na propria planilha",
      perto(sm16["LCG"], 493.5 / 60.0, 1e-9), f"{sm16['LCG']}")
checa("KG confere com o total declarado na propria planilha",
      perto(sm16["KG"], 75.25 / 60.0, 1e-9), f"{sm16['KG']}")
checa("A coluna de momento nao foi confundida com a de LCG",
      perto(float(df16["LCG (m)"].to_numpy(float)[0]), 8.0, 1e-9),
      f"{df16['LCG (m)'].to_numpy(float)[0]}")
checa("Condicao lida de planilha nao gera ERRO na validacao",
      not any(a.nivel == "ERRO" for a in g["validar_condicao"](df16)))
checa("O usuario e avisado do que foi descartado",
      any("IGNORADAS" in n for n in notas16) and any("rodape" in n for n in notas16))

# um item de peso zero e legitimo e nao pode ser descartado
_linhas16b = [["Item", "Peso (t)", "LCG (m)", "VCG (m)"],
              ["Lastro vazio", 0.0, 5.0, 0.4],
              ["Casco", 10.0, 6.0, 1.0]]
_b16 = _io16.BytesIO()
pd.DataFrame(_linhas16b).to_excel(_b16, index=False, header=False)
...
class _Falso16b:
    ...
    def getvalue(self):
        return _b16.getvalue()


df16b, _ = g["ler_condicao"](_Falso16b())
checa("Item de peso zero e mantido (o enunciado admite zero justificavel)",
      len(df16b) == 2, f"{len(df16b)} itens")


# uma nota de rodape que mencione peso e LCG na mesma frase nao pode ser tomada
# por linha de cabecalho
_linhas16c = [
    ["C1 - Leve (somente peso leve)"],
    ["Referencia: AP = 0,000 m, FP = 19,190 m. LCG medido a partir da AP."],
    [None],
    ["Item", "Peso (t)", "LCG (m)", "VCG (m)"],
    ["Embarcacao leve", 48.0, 8.0, 1.25],
]
_b16c = _io16.BytesIO()
pd.DataFrame(_linhas16c).to_excel(_b16c, index=False, header=False)
...
class _Falso16c:
    ...
    def getvalue(self):
        return _b16c.getvalue()


df16c, _ = g["ler_condicao"](_Falso16c())
checa("Nota de rodape citando peso e LCG nao vira cabecalho",
      len(df16c) == 1 and perto(float(df16c["Peso (t)"].to_numpy(float)[0]), 48.0, 1e-9),
      f"{len(df16c)} item(ns)")
checa("Condicao com um unico item e lida corretamente",
      perto(float(df16c["LCG (m)"].to_numpy(float)[0]), 8.0, 1e-9))


# --------------------------------------------------------------------------
# 16d. uma TABELA DE RESULTADOS nao pode ser lida como condicao de carga
# --------------------------------------------------------------------------
# O resumo das condicoes tem colunas "Delta_t" e "LCG_m", que casam com as
# palavras-chave de peso e de LCG. Sem a trava, o app leria as seis linhas de
# resumo como seis itens de peso e devolveria Delta = 381,737 t em silencio.
_linhas16d = [
    ["Condicao", "Delta_t", "LCG_m", "KG_m", "GG0_m", "KG_corrigido_m", "T0_m",
     "LCB_m", "LCF_m", "BMl_m", "MTC", "Mtrim_tm", "trim_cm", "Ta_m", "Tf_m",
     "theta_graus", "GMt_m", "Sentido"],
    ["C1_leve", 48.0, 8.0, 1.25, 0.0, 1.25, 1.1997, 7.9739, 7.9274, 32.737,
     0.8189, 1.252, 1.53, 1.1934, 1.2087, 0.0457, 2.2892, "aproado"],
    ["C2_partida_100", 69.978, 8.4558, 1.2478, 0.0, 1.2478, 1.4798, 8.0029,
     8.183, 25.507, 0.9301, 31.692, 34.07, 1.3345, 1.6752, 1.0172, 1.8827,
     "aproado"],
    ["C5_carga_a_re", 69.978, 7.6700, 1.2478, 0.0, 1.2478, 1.4798, 8.0029,
     8.183, 25.507, 0.9301, -23.31, -25.06, 1.5866, 1.3361, -0.7478, 1.8827,
     "apopado"],
]
_b16d = _io16.BytesIO()
pd.DataFrame(_linhas16d).to_excel(_b16d, index=False, header=False)


class _Falso16d:
    name = "RESUMO_condicoes.xlsx"

    def getvalue(self):
        return _b16d.getvalue()


_recusou16d = False
_msg16d = ""
try:
    g["ler_condicao"](_Falso16d())
except Exception as _e16d:                                  # noqa: BLE001
    _recusou16d = True
    _msg16d = str(_e16d)

checa("Tabela de resultados e recusada como condicao de carga", _recusou16d,
      "o arquivo de resumo foi aceito como condicao")
checa("A recusa explica que o arquivo e uma tabela de resultados",
      "TABELA DE RESULTADOS" in _msg16d, _msg16d[:80])

# e a trava nao pode atrapalhar uma condicao legitima: uma condicao de carga
# comum nunca tem tres ou mais colunas de resultado no cabecalho
_linhas16e = [
    ["Item", "Peso (t)", "LCG (m)", "VCG (m)"],
    ["Embarcacao leve", 48.0, 8.0, 1.25],
    ["Tripulacao", 0.5, 12.0, 3.2],
    ["Combustivel 100%", 7.038, 5.5, 0.75],
]
_b16e = _io16.BytesIO()
pd.DataFrame(_linhas16e).to_excel(_b16e, index=False, header=False)
...
class _Falso16e:
    ...
    def getvalue(self):
        return _b16e.getvalue()


df16e, _ = g["ler_condicao"](_Falso16e())
checa("A trava nao recusa uma condicao de carga legitima", len(df16e) == 3,
      f"{len(df16e)} itens")
_s16e = g["somatorio"](df16e)
checa("Somatorio da condicao legitima continua correto",
      perto(_s16e["delta"], 55.538, 1e-6), f"Delta={_s16e['delta']:.6f} t")


# --------------------------------------------------------------------------
# 17. a navegacao tem de avancar ate o fim, sem voltar nem pular etapa
# --------------------------------------------------------------------------
# Ja aconteceu de uma tela apontar "Proximo passo" para uma etapa ANTERIOR,
# fechando um ciclo 7 -> 8 -> 9 -> 7 do qual o usuario nao conseguia sair pelo
# botao. Este teste le o destino de cada botao direto do codigo-fonte.
import re as _re17                                            # noqa: E402
import pathlib as _pl17                                       # noqa: E402

print("\n[17] Encadeamento das telas")
_dir17 = _pl17.Path(__file__).resolve().parent / "interface"
_paginas17 = []
_txt_comum = (_dir17 / "comum.py").read_text(encoding="utf-8")
_m17 = _re17.search(r"PAGINAS\s*=\s*\[(.*?)\]", _txt_comum, _re17.S)
if _m17:
    _paginas17 = _re17.findall(r'"([^"]+)"', _m17.group(1))
checa("A lista de paginas foi encontrada em comum.py", len(_paginas17) >= 2,
      f"{len(_paginas17)} paginas")

# de que modulo vem cada pagina, lido do roteador em app.py
_app17 = (_pl17.Path(__file__).resolve().parent / "app.py").read_text(encoding="utf-8")
_mod_da_pag17 = {}
for _pag17, _mod17 in _re17.findall(r'"([^"]+)"\s*:\s*(\w+)\.render', _app17):
    _mod_da_pag17[_mod17 + ".py"] = _pag17
checa("O roteador de paginas foi encontrado em app.py",
      len(_mod_da_pag17) == len(_paginas17),
      f"{len(_mod_da_pag17)} modulos para {len(_paginas17)} paginas")

_destinos17 = {}
for _f17 in sorted(_dir17.glob("*.py")):
    if _f17.name in ("comum.py", "__init__.py"):
        continue
    _t17 = _f17.read_text(encoding="utf-8")
    _alvos17 = _re17.findall(r'botao_proximo\(\s*"([^"]+)"', _t17)
    _alvos17 += _re17.findall(r'ir_para\(\s*"([^"]+)"', _t17)
    if _alvos17:
        _destinos17[_f17.name] = _alvos17

_erros17 = []
for _arq17, _lista17 in _destinos17.items():
    for _d17 in _lista17:
        if _d17 not in _paginas17:
            _erros17.append(f"{_arq17} aponta para '{_d17}', que nao e uma pagina")
checa("Todo destino de 'Proximo passo' existe na lista de paginas",
      not _erros17, " | ".join(_erros17))

# o encadeamento tem de ir sempre para a frente
_pos17 = {n: i for i, n in enumerate(_paginas17)}
_retro17 = []
for _arq17, _lista17 in _destinos17.items():
    _origem17 = _mod_da_pag17.get(_arq17)
    if _origem17 is None:
        continue
    for _d17 in _lista17:
        if _d17 in _pos17 and _pos17[_d17] <= _pos17[_origem17]:
            _retro17.append(f"{_arq17} ('{_origem17}') avanca para '{_d17}'")
checa("Nenhum botao de avancar leva a uma etapa anterior", not _retro17,
      " | ".join(_retro17))

# e a ultima pagina precisa ser alcancavel por algum botao
_ultima17 = _paginas17[-1] if _paginas17 else ""
_alcanca17 = any(_ultima17 in v for v in _destinos17.values())
checa(f"A ultima etapa ('{_ultima17}') e alcancavel pelo botao de avancar",
      _alcanca17, "nenhuma tela aponta para a ultima etapa")

# nenhuma etapa intermediaria pode ficar sem quem aponte para ela
_apontadas17 = {d for v in _destinos17.values() for d in v}
_orfas17 = [p for p in _paginas17[1:] if p not in _apontadas17]
checa("Nenhuma etapa fica sem um botao que leve ate ela", not _orfas17,
      " | ".join(_orfas17))
